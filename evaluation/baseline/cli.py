"""Command-line interface for the baseline evaluator.

Usage (from the repository root):

    python -m evaluation.baseline --version
    python -m evaluation.baseline one \
        --evidence evaluation/snapshots/C001-gosim/evidence.yaml \
        --out evaluation/results/local/baseline/C001-baseline.yaml
    python -m evaluation.baseline one --evidence <path> --provider openai-compatible --model <model>
...
Exit codes and YAML-to-stdout conventions mirror the snapshot, evidence, and
scoring CLIs. A failed assessment is printed/written (for the audit record)
and exits non-zero; it is never converted into a score.
"""
...
from evaluation.baseline.models import STATUS_FAILED, STATUS_SUCCEEDED
from evaluation.baseline.paths import default_results_dir
...
from evaluation.baseline.serialize import render_result, write_result
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Deliberately simple single-LLM rubric assessment over a frozen "
    ...
    one = subparsers.add_parser(
        "one", help="Run one baseline assessment for a single evidence artifact."
    )
    ...
    dataset = subparsers.add_parser(
        "dataset", help="Run the baseline over every evidence artifact present in the store."
    )
    ...
    inspect = subparsers.add_parser("inspect", help="Inspect a baseline result artifact.")
...
def _load_evidence(path: Path) -> EvidenceArtifact:
    try:
        ...
        raise BaselineError(f"unreadable evidence artifact: {exc}") from exc
    if not isinstance(raw, dict):
        raise BaselineError("evidence artifact is not a mapping")
    try:
        return artifact_from_dict(raw)
    except (KeyError, TypeError, ValueError) as exc:
        raise BaselineError(f"invalid evidence artifact: {exc}") from exc
...
def _cmd_dataset(args: argparse.Namespace) -> int:
    ...
    for evidence_path in _discover_evidence(args.store):
        try:
            ...
            continue
        out = args.results_dir / f"{evidence.case_id}-baseline-v{result.baseline_version}.yaml"
...
def _cmd_inspect(args: argparse.Namespace) -> int:
    try:
        ...
        return 1
    from evaluation.baseline.serialize import recompute_identity
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (BaselineError, EvidenceError) as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 1
